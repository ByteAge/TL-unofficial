...
public class ChatBaseCell extends BaseCell implements MediaController.FileDownloadProgressListener {
    ...
    public ColorUtils.ColorPresenter colorPresenter;
    ...
    public ChatBaseCell(Context context) {
        ...
        colorPresenter=getColorPresenter();
    }

    public ColorUtils.ColorPresenter getColorPresenter(){
        return ChitSettings.showFantasy ? ColorUtils.fantasyColorPresenter :  ColorUtils.defaultColorPresenter;
    }
    ...
    protected void measureTime(MessageObject messageObject) {
        ...
        if ((messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_HAS_VIEWS) != 0) {
            ...
            timeWidth += (int) Math.ceil(timePaint.measureText(currentViewsString)) + colorPresenter.viewsIn.getIntrinsicWidth() + AndroidUtilities.dp(10);
        }
    }
    ...
    public void setMessageObject(MessageObject messageObject) {
        ...
        if ((messageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_HAS_VIEWS) != 0) {
            ...
            timeWidth += viewsTextWidth + colorPresenter.viewsIn.getIntrinsicWidth() + AndroidUtilities.dp(10);
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (media) {
            ...
            if (currentMessageObject.isOutOwner()) {
                timePaint.setColor(//0xff70b15c
                        colorPresenter.timeOut
                );
            } else {
                timePaint.setColor(isDrawSelectedBackground() ? //0xff89b4c1 : 0xffa1aab3
                                colorPresenter.timeInSelected : colorPresenter.timeIn
                );
            }
        }
        ...
        if (currentMessageObject.isOutOwner()) {
            if (isDrawSelectedBackground()) {
                if (!media) {
                    currentBackgroundDrawable = colorPresenter.msgOutSelected;//ResourceLoader.backgroundDrawableOutSelected;
                } else {
                    currentBackgroundDrawable = colorPresenter.mediaOutSelected;
                }
            } else {
                if (!media) {
                    currentBackgroundDrawable = colorPresenter.msgOut;//ResourceLoader.backgroundDrawableOut;
                } else {
                    currentBackgroundDrawable = colorPresenter.mediaOut;
                }
            }
            ...
            if (isDrawSelectedBackground()) {
                if (!media) {
                    currentBackgroundDrawable = colorPresenter.msgInSelected;//ResourceLoader.backgroundDrawableInSelected;
                } else {
                    currentBackgroundDrawable = colorPresenter.mediaInSelected;
                }
            } else {
                if (!media) {
                    currentBackgroundDrawable = colorPresenter.msgIn;//ResourceLoader.backgroundDrawableIn;
                } else {
                    currentBackgroundDrawable = colorPresenter.mediaIn;
                }
            }
        ...
        if (drawForwardedName && forwardedNameLayout != null) {
            ...
            if (currentMessageObject.isOutOwner()) {
                forwardNamePaint.setColor(colorPresenter.forwardOutColor);
                forwardNameX = currentBackgroundDrawable.getBounds().left + AndroidUtilities.dp(10);
            } else {
                forwardNamePaint.setColor(colorPresenter.forwardInColor);
                if (media) {
                    forwardNameX = currentBackgroundDrawable.getBounds().left + AndroidUtilities.dp(10);
                } else {
        ...
        if (currentMessageObject.isReply()) {
            if (currentMessageObject.type == 13) {
                ...
                replyLinePaint.setColor(//0xffffffff
                    colorPresenter.replyLineSticker
                );
                replyNamePaint.setColor(//0xffffffff
                        colorPresenter.replySticker
                );
                replyTextPaint.setColor(//0xffffffff
                        colorPresenter.replyTextSticker
                );
                int backWidth;
                if (currentMessageObject.isOutOwner()) {
                ...
                if (currentMessageObject.isOutOwner()) {
                    replyLinePaint.setColor(//0xff8dc97a
                            colorPresenter.replyLineOut
                    );
                    replyNamePaint.setColor(//0xff61a349
                            colorPresenter.replyOut
                    );
        ...
        if (drawTime || !media) {
            if (media) {
                ...
                if ((currentMessageObject.messageOwner.flags & TLRPC.MESSAGE_FLAG_HAS_VIEWS) != 0) {
                    ...
                    if (currentMessageObject.isSending()) {
                        ...
                        if (!currentMessageObject.isOutOwner()) {
                            setDrawableBounds(colorPresenter.viewsIn, timeX, layoutHeight - AndroidUtilities.dp(4.5f) - timeLayout.getHeight());
                            colorPresenter.viewsIn.draw(canvas);
                        } else {
                            setDrawableBounds(colorPresenter.viewsOut, timeX, layoutHeight - AndroidUtilities.dp(4.5f) - timeLayout.getHeight());
                            colorPresenter.viewsOut.draw(canvas);
                        }

                        if (viewsLayout != null) {
                            canvas.save();
                            canvas.translate(timeX + colorPresenter.viewsOut.getIntrinsicWidth() + AndroidUtilities.dp(3), layoutHeight - AndroidUtilities.dp(6.5f) - timeLayout.getHeight());
                            viewsLayout.draw(canvas);
                            canvas.restore();
                        }
                    }
            ...
            if (currentMessageObject.isOutOwner()) {
                ...
                if (drawClock) {
                    if (!media) {
                        setDrawableBounds(colorPresenter.clock, layoutWidth - AndroidUtilities.dp(18.5f) - colorPresenter.clock.getIntrinsicWidth(), layoutHeight - AndroidUtilities.dp(8.5f) - colorPresenter.clock.getIntrinsicHeight());
                        colorPresenter.clock.draw(canvas);
                    } else {
                ...
                if (isBroadcast) {
                    if (drawCheck1 || drawCheck2) {
                        if (!media) {
                            setDrawableBounds(colorPresenter.broadcast, layoutWidth - AndroidUtilities.dp(20.5f) - colorPresenter.broadcast.getIntrinsicWidth(), layoutHeight - AndroidUtilities.dp(8.0f) - colorPresenter.broadcast.getIntrinsicHeight());
                            colorPresenter.broadcast.draw(canvas);
                        } else {
                    ...
                    if (drawCheck2) {
                        if (!media) {
                            if (drawCheck1) {
                                setDrawableBounds(colorPresenter.checkD, layoutWidth - AndroidUtilities.dp(22.5f) - colorPresenter.checkD.getIntrinsicWidth(), layoutHeight - AndroidUtilities.dp(8.0f) - colorPresenter.checkD.getIntrinsicHeight());
                            } else {
                                setDrawableBounds(colorPresenter.checkD, layoutWidth - AndroidUtilities.dp(18.5f) - colorPresenter.checkD.getIntrinsicWidth(), layoutHeight - AndroidUtilities.dp(8.0f) - colorPresenter.checkD.getIntrinsicHeight());
                            }
                            colorPresenter.checkD.draw(canvas);
                        } else {
                            if (drawCheck1) {
                    ...
                    if (drawCheck1) {
                        if (!media) {
                            Drawable half=colorPresenter.halfCheckD;
                            setDrawableBounds(half, layoutWidth - AndroidUtilities.dp(18) - half.getIntrinsicWidth(), layoutHeight - AndroidUtilities.dp(8.0f) - half.getIntrinsicHeight());
                            half.draw(canvas);
                        } else {
